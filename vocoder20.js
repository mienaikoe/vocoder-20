...
n.msGetUserMedia);n.getUserMedia({audio:true},function(stream){
 mic=c.createMediaStreamSource(stream);for(var i=0;i<4;i++){
  ...
  var az=c._G(); az.gain.value=0.2; oz.to(az);ox[i]=az;}
cv=new Float32Array(fm);for(i=-fh;i<fh;i++){cv[i+fh]=((i>0)?i:-i)/fh;}
for(i=0;i<8;i++){var mb=c._F();mb.type="bandpass";
mb.frequency.value=55*Math.pow(2,(i/2));var mr=c._W();mr.curve=cv;var mf=c._F();
mf.type='LOWPASS';mf.frequency.value=20;var ob=c._F();ob.type="bandpass";
ob.frequency.value=55*Math.pow(2,(i/2));var og=c._G();og.gain.value=0.0;
