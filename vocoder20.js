...
mic=c.createMediaStreamSource(stream);
for(var i=0;i<26;i++){oz=c.createOscillator();oz.type='sine';oz.start();
...
window.ael("keydown",function(ev){ox[(ev.keyCode+13)%26].gain.value=0.5;});
window.ael("keyup",	 function(ev){ox[(ev.keyCode+13)%26].gain.value=0.0;});
},function(){});